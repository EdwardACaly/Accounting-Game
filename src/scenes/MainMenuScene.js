...
export class MainMenuScene extends Scene {
  ...
  create() {
    ...
    const createButton = (x, y, labelText, onClick) => {
      ...
      const border = this.add.rectangle(0, 0, 604, 124, 0x7f1a02).setDepth(3);
      border.setStrokeStyle(3, 0xdcc89f);

      // button hitbox
      const rect = this.add.rectangle(0, 0, 600, 120, 0x7f1a02).setDepth(3);
      ...
      // version of label using bitmap font over .ttl format (looks worse btw)
      /*
      const label = this.add.bitmapText(0, 0, "jersey10", labelText, 118)
        .setOrigin(0.5)
        .setDepth(3)
        .setTint(0xdcc89f);
      */
    ...
    const startY = (height / 2 - blockHeight / 2) + 100;
    ...
    // --- Icon constants ---
    const ICON_Y = 100;   // y-pos of icons
    const ICON_MARGIN = 90;     // distance from border
    const BASE_SCALE = 0.15;      // Base scale (size) for all icons
